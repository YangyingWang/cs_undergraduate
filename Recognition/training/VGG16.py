...
OUT_PUT_MODEL_FT = '/root/Recognition/models/vgg16.pth'  # 微调后的模型保存路径
EPOCHS = 75
BATCH_SIZE = 16
...
def plot_training(history):
    ...
    plt.legend()
    plt.grid()
    plt.savefig("/root/Recognition/figs/VGG16/accuracy.png")
    ...
    plt.legend()
    plt.grid()
    plt.savefig("/root/Recognition/figs/VGG16/loss.png")
    ...
    plt.legend()
    plt.grid()
    plt.savefig("/root/Recognition/figs/VGG16/f1_score.png")
...
def train(args):
    ...
    for epoch in range(epochs):
        ...
        # 输出训练和验证的损失与准确率
        print(f"[Epoch {epoch + 1}/{epochs}] | Train Loss: {running_loss/len(train_loader):.4f} | Train Acc: {train_acc:.4f} | Val Loss: {val_loss:.4f} | Val Acc: {val_acc:.4f}")
